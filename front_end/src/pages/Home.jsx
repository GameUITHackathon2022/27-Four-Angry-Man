...
const Home = () => {
        ...
        const json = await response.json()
            //console.log(json)
                const cardTmp = json["data"].map((data1) => {
                    console.log(data1)
                    return (
                        <CardInfo key={data1["id"]}
                        status={data1["status"]}
                        ward={data1["ward"]} 
                        district={data1["district"]}
                        description={data1["description"]}
                        image_url={data1["image_url"]}></CardInfo>
                    )
                })
                setCard(cardTmp)
    ...
    <Box sx={{display: 'flex', flexDirection:'column', justifyContent: 'space-evenly'}}>
      <Box className='input-box-home'>
        <input className='input-select-home' type="text" disabled={true} value='TP Hồ Chí Minh'/>
        <select className="form-select input-select-home" onChange={(e)=>handleDistrict(e)}>
        {
        ...
        <select className="form-select input-select-home"  onChange={(e)=>handleWard(e)}>
        {
            Wards.map((Ward)=>(
        ...
        <IconButton color="primary" aria-label="search" sx={{marginTop:'10px'}} onClick={handleFilter}>
